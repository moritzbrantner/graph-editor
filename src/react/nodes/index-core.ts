...
export type GraphNodeTypeScriptType<TypeScriptType = unknown> =
  | string
  | {
      label?: string;
      source?: string;
...
export type GraphNodePort<TypeScriptType = unknown> = {
  id: string;
  label: string;
  kind?: string;
...
export type GraphInputOnlyNodeData<
  Inputs extends readonly GraphNodePort[] = readonly GraphNodePort[],
...
export type GraphOutputOnlyNodeData<
  Outputs extends readonly GraphNodePort[] = readonly GraphNodePort[],
...
export type GraphNodeSize = GraphEditorNodeSize;

export type GraphNodeLayoutOptions = GraphEditorNodeLayoutOptions;

export type GraphNodeMenuItem = {
  id: string;
...
export type GraphInputOnlyNodeProps<
  Inputs extends readonly GraphNodePort[] = readonly GraphNodePort[],
> = Omit<
...
export type GraphOutputOnlyNodeProps<
  Outputs extends readonly GraphNodePort[] = readonly GraphNodePort[],
> = Omit<
...
export function getGraphNodeToneFromStatus(status?: string): GraphNodeData["tone"] {
  if (status === "success") {
    return "success";
  }
  if (status === "warning") {
...
export function getGraphNodePortBadgeTextColor(color: string) {
  void color;
  return "#18181b";
}
